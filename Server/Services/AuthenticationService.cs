...
public sealed class AuthenticationService : MagicServerBase<IAuthenticationService>, IAuthenticationService
{
    ...
    public MagicTInMemoryContext InMemoryContext  { get; set; } 
    ...
    public AuthenticationService(IServiceProvider provider) : base(provider)
    {
        ...
        InMemoryContext= provider.GetService<MagicTInMemoryContext>();
    ...
    public async UnaryResult<LoginResponse> LoginWithPhoneAsync(LoginRequest loginRequest)
    {
        return await ExecuteAsync(async () =>
        {
            ...
            var existing = InMemoryContext.UsersCredentials.FirstOrDefault(x => x.UserId == user.U_ROWID);
            
            if(existing is null) 
                InMemoryContext.UsersCredentials.Add(credentials);
            else
                InMemoryContext.UsersCredentials.Update(credentials);
	...
	public async UnaryResult<LoginResponse> LoginWithUsername(LoginRequest loginRequest)
	{
        return await ExecuteAsync(async () =>
        {
            ...
            var existing = InMemoryContext.UsersCredentials.AsNoTracking().FirstOrDefault(x => x.UserId == user.U_ROWID);
            ...
            // InMemoryContext.ChangeTracker.t
