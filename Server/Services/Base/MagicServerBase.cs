...
public abstract partial class MagicServerBase<TService> : ServiceBase<TService> ,IDisposable,IAsyncDisposable where TService : IService<TService>
{
    ...
    protected byte[] SharedKey => MagicTInMemoryContext.UsersCredentials.FirstOrDefault(x => x.UserId == CurrentUserId)?.SharedKey;

    [EnableAutomaticDispose]
    public MagicTInMemoryContext MagicTInMemoryContext { get; set; }
    ...
    protected MagicServerBase(IServiceProvider provider)
    {
        ...
        MagicTInMemoryContext = provider.GetService<MagicTInMemoryContext>();
        ...
        [CallerLineNumber] int callerLineNumber = default) 
    {
        ...
        catch (UniqueConstraintException ex)
        {
            ...
            LogManager.LogError(CurrentUserId, $"A unique constraint violation occurred: {ex.Message}", callerFilePath,
                callerMemberName, callerLineNumber);
            throw new ReturnStatusException(StatusCode.Cancelled,
                $"A unique constraint violation occurred: {ex.Message}");
        }
        catch (ReferenceConstraintException ex)
        {
            ...
            LogManager.LogError(CurrentUserId, $"A reference constraint violation occurred: {ex.Message}",
                callerFilePath, callerMemberName, callerLineNumber);
            throw new ReturnStatusException(StatusCode.Cancelled,
                $"A reference constraint violation occurred: {ex.Message}");
        }
        catch (CannotInsertNullException ex)
        {
            ...
            LogManager.LogError(CurrentUserId, $"A not null constraint violation occurred: {ex.Message}",
                callerFilePath, callerMemberName, callerLineNumber);
            throw new ReturnStatusException(StatusCode.Cancelled,
                $"A not null constraint violation occurred: {ex.Message}");
        ...
        catch (MaxLengthExceededException ex)
        {
            ...
            LogManager.LogError(CurrentUserId, $"A max length constraint violation occurred: {ex.Message}",
                callerFilePath, callerMemberName, callerLineNumber);
            throw new ReturnStatusException(StatusCode.Cancelled,
                $"A max length constraint violation occurred: {ex.Message}");
        }
        catch (ReturnStatusException ex)
        {
            ...
            throw new ReturnStatusException(StatusCode.Cancelled, ex.Detail);
        }
        catch (Exception ex)
        {
            if (Transaction is not null)
            ...
            LogManager.LogError(CurrentUserId, ex.Message,callerFilePath, callerMemberName, callerLineNumber);
            throw new ReturnStatusException(StatusCode.Cancelled, ex.Message);
        }
